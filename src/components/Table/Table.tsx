...
import {
    ...
    IModifiedColumns,
    RootState,
    // Убедитесь, что остальные интерфейсы определены корректно
} from '../../interfaces';

// Определение StateProps
...
// Определение OwnProps (если есть)
interface OwnProps {
    // Например, className?: string;
}

// Combine пропсов с помощью ConnectedProps
const mapStateToProps = (state: RootState): StateProps => ({
    columns: state.columns,
    data: state.data,
    columnTypes: state.columnTypes,
...
const TableComponent: React.FC<Props> = ({
                                             ...
                                             updateData,
                                             setColumnType,
                                             // ... любые дополнительные пропсы из OwnProps
                                         }) => {
    // Ваш код компонента
    const [isModalVisible, setIsModalVisible] = useState(false);
    const [isEditModalVisible, setIsEditModalVisible] = useState(false);
...
// Экспорт подключенного компонента
